...
const AdminDetailsDialog = ({ open, onClose, adminDetails }) => {
  const capitalizedAdminName = adminDetails.name.charAt(0).toUpperCase() + adminDetails.name.slice(1).toLowerCase();
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      <DialogTitle>Admin Details</DialogTitle>
      <DialogContent>
        {/* <Typography variant="body1">Name: {adminDetails.name.charAt(0).toUpperCase() + adminDetails.name.slice(1)}</Typography> */}
